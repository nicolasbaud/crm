 ...
 *
 * @author Nicolas BAUD <nicolas.baud12@gmail.com>
 */
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Install CRM';
    ...
    {
        if ($this->confirm('Do you want to configure CRM before installing?')) {
    ...
    protected function configureDatabase(array $default = [])
    {
        ...
        if ($config['DB_DRIVER'] === 'sqlite') {
            ...
            $config['DB_DATABASE'] = $this->ask('What is the name of the database that CRM should use?', $config['DB_DATABASE']);
    ...
    protected function configureMail(array $config = [])
    {
        ...
        if (!$this->confirm('Do you want configure mail notifications?')) {
            return;
        }
    ...
    protected function configureRedis()
    {
        ...
        $config['REDIS_DATABASE'] = $this->ask('What is the name of the database that CR% should use?');
        $config['REDIS_PORT'] = $this->ask('What port should CRM use?', 6379);
